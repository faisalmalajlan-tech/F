...
(function () {
  ...
  /* ═══════════ الحسابات ═══════════
     أربعة حسابات: مدير + ثلاثة مستخدمين. الرموز مخزّنة كبصمة لا كنص،
     لكن ذلك تعمية لا حماية: الملف يعمل بلا خادم، فمن يفتحه بمحرر
     يستطيع تجاوز كل هذا. الغرض فصل الأدوار وتتبّع المسؤولية. */
  var ACC = (function () {
    var KEY = 'nadheer:accounts';
    var SEED = [
      { id: 'admin',  name: 'مدير النظام', role: 'admin', code: 'ADMIN1', active: true },
      { id: 'faisal', name: 'فيصل',        role: 'user',  code: '1234FA', active: true },
      { id: 'rawan',  name: 'روان',        role: 'user',  code: 'RA12',   active: true },
      { id: 'safiah', name: 'صفية',        role: 'user',  code: '1313SA', active: true }
    ];
    function load() {
      try {
      ...
      var seeded = SEED.map(function (a) {
        return { id: a.id, name: a.name, role: a.role, hash: codeHash(a.code), active: a.active, seedCode: a.code };
      });
      save(seeded);
      return seeded;
    }
    ...
    function get(id) {
      var f = load().filter(function (a) { return a.id === id; });
      return f.length ? f[0] : null;
    }
    function verify(id, code) {
      var a = get(id);
      return !!(a && a.active && codeHash(code || '') === a.hash);
    }
    function setCode(id, code) {
      var list = load();
      list.forEach(function (a) { if (a.id === id) { a.hash = codeHash(code); delete a.seedCode; } });
      save(list);
    }
    function setActive(id, on) {
      var list = load();
      list.forEach(function (a) { if (a.id === id && a.id !== 'admin') a.active = !!on; });
      save(list);
    }
    return { all: all, get: get, verify: verify, setCode: setCode, setActive: setActive };
  })();

  /* ═══════════ سجل النسخ ═══════════
     لقطة كاملة قبل كل تغيير مؤثر — هي ما يجعل «الإلغاء» ممكنًا فعلًا. */
  var VS = (function () {
    var KEY = 'nadheer:versions', MAX = 40;
    function load() { try { return JSON.parse(window.localStorage.getItem(KEY) || '[]'); } catch (e) { return []; } }
    function save(l) { try { window.localStorage.setItem(KEY, JSON.stringify(l.slice(-MAX))); } catch (e) {} }
    function snap(label, actor) {
      var docs;
      try { docs = JSON.parse(window.localStorage.getItem('nadheer:docs:v1') || '[]'); } catch (e) { docs = []; }
      var v = { id: 'v' + Date.now() + Math.random().toString(36).slice(2, 6),
                at: Date.now(), label: label || '', actor: actor || '—', docs: docs };
      var l = load(); l.push(v); save(l);
      return v.id;
    }
    function restore(vid) {
      var l = load(), hit = null;
      l.forEach(function (v) { if (v.id === vid) hit = v; });
      if (!hit) return false;
      snap('قبل الاسترجاع', S.session ? S.session.name : '—');
      try { window.localStorage.setItem('nadheer:docs:v1', JSON.stringify(hit.docs)); } catch (e) { return false; }
      if (ST.reset) ST.reset();        // تصفير الكاش حتى تُقرأ النسخة المسترجَعة
      return true;
    }
    return { all: load, snap: snap, restore: restore,
             clear: function () { try { window.localStorage.removeItem(KEY); } catch (e) {} } };
  })();

  /* ═══════════ طلبات الموافقة ═══════════
     المستخدم يعمل بحرية، والإجراء يُنفَّذ فورًا — ثم يصل المدير إشعار
     يقبله أو يلغيه. الإلغاء يرجّع اللقطة السابقة، فلا يكون القبول
     مجرد كلام. الإجراءات القرائية لا تُسجَّل هنا حتى لا يغرق الصندوق. */
  var RQ = (function () {
    var KEY = 'nadheer:requests', MAX = 200;
    function load() { try { return JSON.parse(window.localStorage.getItem(KEY) || '[]'); } catch (e) { return []; } }
    function save(l) { try { window.localStorage.setItem(KEY, JSON.stringify(l.slice(-MAX))); } catch (e) {} }
    function record(o) {
      if (!S.session || S.session.role === 'admin') return null;   // فعل المدير لا يحتاج موافقة نفسه
      var l = load();
      l.push({ id: 'r' + Date.now() + Math.random().toString(36).slice(2, 5),
               at: Date.now(), actor: S.session.name, actorId: S.session.id || '',
               action: o.action || 'إجراء', title: o.title || '', detail: o.detail || '',
               code: o.code || '', versionId: o.versionId || '', status: 'pending',
               decidedAt: 0, decidedBy: '' });
      save(l);
      return true;
    }
    function pending() { return load().filter(function (r) { return r.status === 'pending'; }).reverse(); }
    function decide(id, ok) {
      var l = load(), hit = null;
      l.forEach(function (r) { if (r.id === id) hit = r; });
      if (!hit || hit.status !== 'pending') return { ok: false };
      var reverted = false;
      if (!ok && hit.versionId) reverted = VS.restore(hit.versionId);
      hit.status = ok ? 'approved' : 'reverted';
      hit.decidedAt = Date.now();
      hit.decidedBy = S.session ? S.session.name : '—';
      save(l);
      AU.log({ kind: 'security', actor: S.session ? S.session.name : '—', code: hit.code,
               title: ok ? 'اعتماد إجراء' : 'إلغاء إجراء',
               detail: hit.actor + ' · ' + hit.title + (!ok ? (reverted ? ' · أُرجعت الحالة السابقة' : ' · بلا لقطة للإرجاع') : '') });
      return { ok: true, reverted: reverted };
    }
    return { record: record, pending: pending, all: load, decide: decide,
             clear: function () { try { window.localStorage.removeItem(KEY); } catch (e) {} } };
  })();
  ...
  function renderGate() {
    ...
    $('gate').innerHTML = '<div class="gate-card">' +
      ...
      '<div class="roles">' +
      accs.map(function (a) {
        return '<button type="button" class="role' + (gateRole === a.id ? ' sel' : '') +
          '" data-acc="' + a.id + '"' + (a.active ? '' : ' disabled style="opacity:.4"') + '>' +
          ico(a.role === 'admin' ? 'shield' : 'user', 21) +
          '<div class="rn">' + esc(a.name) + '</div>' +
          '<div class="rd">' + (a.role === 'admin' ? 'صلاحية كاملة ومراجعة' : 'تحليل ومتابعة') + '</div></button>';
      }).join('') +
    ...
    on($('gate'), '.role', 'click', function (b) { gateRole = b.dataset.acc; renderGate(); });
    var cur = ACC.get(gateRole);
    $('gateFields').innerHTML = cur
      ? '<label class="field-label" for="gateCode">رمز ' + esc(cur.name) + '</label>' +
        '<input type="password" id="gateCode" autocomplete="off">' +
        (cur.seedCode ? '<div style="font-size:10px;color:var(--warning);margin-top:8px;line-height:1.7">الرمز الأولي ما زال فعّالًا — غيّره من صفحة الأمان.</div>' : '')
      : '<div style="font-size:11px;color:var(--text2);margin-top:10px">اختر حسابًا للمتابعة.</div>';
    var doGo = function () {
      var a = ACC.get(gateRole);
      if (!a) { $('gateErr').innerHTML = '<div class="errbox">اختر حسابًا أولًا.</div>'; return; }
      if (!a.active) { $('gateErr').innerHTML = '<div class="errbox">هذا الحساب موقوف.</div>'; return; }
      if (!ACC.verify(a.id, $('gateCode').value || '')) {
        $('gateErr').innerHTML = '<div class="errbox">رمز غير صحيح.</div>';
        AU.log({ kind: 'security', actor: a.name, title: 'محاولة دخول فاشلة', detail: 'رمز غير صحيح' });
        return;
      }
      S.session = { id: a.id, role: a.role, name: a.name, loginAt: Date.now() };
    ...
    $('gateGo').addEventListener('click', doGo);
    var inp = $('gateCode');
    if (inp) { inp.addEventListener('keydown', function (e) { if (e.key === 'Enter') doGo(); }); inp.focus(); }
  }
  function fmtDur(ms) {
    var s = Math.max(0, Math.round(ms / 1000)), hh = Math.floor(s / 3600),
        mm = Math.floor((s % 3600) / 60), ss = s % 60;
    return (hh ? hh + ' س ' : '') + (hh || mm ? mm + ' د ' : '') + ss + ' ث';
  }
  function logout() {
    var dur = S.session && S.session.loginAt ? fmtDur(Date.now() - S.session.loginAt) : '';
    AU.log({ kind: 'auth', actor: S.session ? S.session.name : '—', title: 'تسجيل خروج',
             detail: dur ? 'مدة الجلسة: ' + dur : '' });
  ...
  var NAV = [
    ...
    { r: 'approvals', t: 'الموافقات والنسخ', admin: true },
  ...
  function go(route, arg) {
    ...
    if (['admin', 'audit', 'security', 'approvals'].indexOf(route) > -1 && !isAdmin()) route = 'home';
  ...
  function wire(el) {
    ...
    on(el, '[data-status]', 'click', function (b) {
      guard('task', 'تحديث مهمة معالجة', 'الحالة: ' + STATUS[b.dataset.status], b.dataset.code);
  ...
  function renderDocs() {
    ...
    on(el, '[data-del]', 'click', function (b) {
      ...
      guard('delete', 'حذف مستند', d ? d.name : '', d ? d.caseCode : '');
  ...
  function tabFix(doc, r) {
    var body = $('tabBody');
    if (!doc.refText) {
      var isReg = doc.role === 'regulatory';
      body.innerHTML = '<div class="card"><div class="empty">' +
        (isReg
          ? 'هذا مستند جهة رقابية — فهو المرجع نفسه.<br>الصق سياستك الداخلية هنا لتُقارن به ويُكشف ما يخالفه.'
          : 'كشف التعارض يحتاج مرجعًا تنظيميًا لمقارنة سياستك به.<br>الصق نص المرجع هنا ليُفحص المستند فورًا.') +
        '</div>' +
        '<textarea id="addRef" placeholder="' + (isReg ? 'الصق نص سياستك الداخلية...' : 'الصق نص المرجع التنظيمي...') + '" style="min-height:120px"></textarea>' +
      ...
      $('saveRef').addEventListener('click', function () {
        ...
        if (!t) return;
        /* لمستند رقابي: النص المُلصق هو السياسة، والمستند الحالي يصير المرجع */
        if (isReg) ST.update(doc.id, { refText: doc.text.slice(0, ST.MAX_TEXT), text: t.slice(0, ST.MAX_TEXT) });
        else ST.update(doc.id, { refText: t.slice(0, ST.MAX_TEXT) });
        guard('ref', 'إضافة مرجع تنظيمي', doc.name, doc.caseCode);
        AU.log({ kind: 'data', actor: S.session.name, code: doc.caseCode,
  ...
  function renderUpload() {
    ...
    el.innerHTML = banner() +
      ...
      '<label class="field-label" for="docRole">دور المستند — من أصدره</label>' +
      '<select id="docRole">' +
      '<option value="internal"' + (S.draft.role === 'regulatory' ? '' : ' selected') + '>مستند داخلي (سياسة · لائحة · عقد) — يُقارَن بمرجع</option>' +
      '<option value="regulatory"' + (S.draft.role === 'regulatory' ? ' selected' : '') + '>مستند جهة رقابية (تعميم · تنظيم) — هو المرجع</option>' +
      '</select>' +
      '<div class="doclabel" style="margin-top:18px" id="lbl_doc1">المستند — مطلوب</div>' +
      ...
      '<hr class="hair">' +
      '<div class="doclabel" id="lbl_doc2">المرجع التنظيمي — اختياري</div>' +
      '<div class="hintline">يُقارَن به كل ملف من الملفات أعلاه. اختره قبلها إن أمكن.</div>' +
      ...
      '<div style="margin-top:16px"><button class="btn" id="analyzeBtn">حلّل واحفظ</button></div>' +
    ...
    // دور المستند يقلب معنى الخانتين — لا يعيد رسم الشاشة ولا يفقد ما أُدخل
    function applyRole() {
      var reg = $('docRole').value === 'regulatory';
      S.draft.role = reg ? 'regulatory' : 'internal';
      $('lbl_doc1').textContent = reg ? 'التعميم أو التنظيم — مطلوب' : 'المستند — مطلوب';
      $('lbl_doc2').textContent = reg ? 'سياستك الداخلية — اختياري (تُقارن بالتعميم)' : 'المرجع التنظيمي — اختياري';
    }
    $('docRole').addEventListener('change', applyRole);
    applyRole();
  ...
  function showFile(key, name, len) {
    ...
    info.querySelector('button').addEventListener('click', function () {
      delete S.files[key]; S.draft[key] = '';
      var box = $('paste_' + key); if (box) box.value = '';
      info.innerHTML = '';
    });
    var nameEl = $('docName');
    if (key === 'doc1' && nameEl && !nameEl.value) { nameEl.value = name.replace(/\.[^.]+$/, ''); S.draft.name = nameEl.value; }
  }
  /* ── الدفعة ──
  ...
  function autoRun() {
    ...
    if (box && !box.querySelector('.auto-note')) {
      ...
      var noRef = !(S.files.doc2 || (S.draft.doc2 || '').trim());
      n.innerHTML = '<span>سيبدأ التحليل والحفظ تلقائيًا…' +
                    (noRef ? ' <b>بلا مرجع للمقارنة</b> — أوقفه إن أردت إضافته.' : '') +
                    '</span><button type="button" class="auto-stop">إيقاف</button>';
      box.appendChild(n);
  ...
  function handleFile(file, key) {
    ...
    (isPdf ? extractPdf(file) : file.text()).then(function (text) {
      ...
      S.draft[key] = text;                         // مصدر واحد للحقيقة
      var box = $('paste_' + key); if (box) box.value = text;
      // رسم الواجهة معزول: خطأ هنا لا يجوز أن يحذف نصًا قُرئ بنجاح
      try { showFile(key, file.name, text.length); }
      catch (uiErr) { info.innerHTML = '<div class="fileinfo"><span>' + esc(file.name) + ' — ' + text.length.toLocaleString('en-US') + ' حرف</span></div>'; }
    }).catch(function (err) {
      info.innerHTML = '';
  ...
  function runBatch(refText) {
    ...
    (function step() {
      ...
      try {
        ...
        /* كل مستند في الدفعة إجراءٌ مؤثر مستقل: لقطة وطلب موافقة لكلٍّ
           منه، وإلا مرّت الدفعة كلها من غير رقابة. */
        guard('analyze', 'تحليل وحفظ مستند: ' + nm, docType, code);
  ...
  function runAnalysis() {
    ...
    // المصدر: ما يراه المستخدم في المربع، ثم المسودة، ثم الملف — أول غير فارغ
    function pick(key) {
      var box = $('paste_' + key), v = box ? (box.value || '').trim() : '';
      if (v) return v;
      if (S.draft[key] && S.draft[key].trim()) return S.draft[key].trim();
      return S.files[key] ? S.files[key].text.trim() : '';
    }
    var t1 = pick('doc1'), t2 = pick('doc2');
    /* دفعةٌ من الملفات؟ كلها تُقارن بالمرجع نفسه.
       ودورُ المستند يبقى محترمًا: إن كان المرفوع تعاميمَ رقابية، فكلٌّ
       منها مرجعٌ يُقاس عليه ما في خانة المقارنة. */
    if (S.batch.length) { runBatch(t2); return; }
    if (!t1) { showError('أضف المستند أولًا — ارفع ملفًا أو الصق النص.'); return; }
    var role = $('docRole') ? $('docRole').value : 'internal';
    /* مستند الجهة الرقابية هو المرجع لا الطرف المحكوم عليه.
       فإن رُفعت معه سياسة داخلية، تُقلب الوجهة: السياسة تُفحص، والتعميم يُقاس عليه. */
    var mainText = t1, refText = t2, isReg = (role === 'regulatory');
    if (isReg && t2) { mainText = t2; refText = t1; }
    ...
    setTimeout(function () {
      var res;
      try { res = E.analyze({ docText: mainText, refText: refText, config: S.cfg, docType: docType }); }
      catch (e) {
        $('proc').classList.remove('active'); go('upload');
        showError('تعذّر تحليل هذا المستند: ' + e.message); return;
      }
      var code = AU.nextCode('NR');
      guard('analyze', 'تحليل وحفظ مستند: ' + name, docType, code);
      var saved = ST.add({ name: name, text: mainText, refText: refText, docType: docType, role: role,
  ...
  /* ═══ الموافقات والنسخ — للمدير وحده ═══ */
  function renderApprovals() {
    /* العنصر اسمه pageContent لا page — كان $('page') يعيد null فتُخفق
       الكتابة صامتةً وتبقى الصفحة السابقة معروضة. */
    var el = $('pageContent'), pend = RQ.pending(), hist = RQ.all().filter(function (r) { return r.status !== 'pending'; }).reverse().slice(0, 30);
    var vers = VS.all().slice().reverse();
    var html = '<h2 class="pagetitle">الموافقات والنسخ</h2>' +
      '<p class="pagesub">كل إجراء مؤثر يقوم به مستخدم يُنفَّذ فورًا ويصل هنا. الاعتماد يُثبته، والإلغاء يرجّع الحالة السابقة فعليًا.</p>';

    html += '<div class="card"><div class="sectitle">بانتظار قرارك (' + pend.length + ')</div>';
    html += pend.length ? pend.map(function (r) {
      return '<div class="fileinfo" style="align-items:flex-start;flex-direction:column;gap:8px;margin-bottom:10px">' +
        '<div style="width:100%"><div style="font-weight:700;font-size:12.5px">' + esc(r.title) + '</div>' +
        '<div style="font-size:11px;color:var(--text2);margin-top:3px;line-height:1.7">' +
          esc(r.actor) + ' · ' + new Date(r.at).toLocaleString('ar-SA') +
          (r.code ? ' · ' + esc(r.code) : '') + (r.detail ? '<br>' + esc(r.detail) : '') + '</div></div>' +
        '<div style="display:flex;gap:8px;width:100%">' +
        '<button class="btn" style="padding:8px" data-ok="' + r.id + '">اعتماد</button>' +
        '<button class="btn ghost" style="padding:8px" data-no="' + r.id + '">إلغاء وإرجاع</button>' +
        '</div></div>';
    }).join('') : '<div class="empty">لا شيء بانتظار قرارك.</div>';
    html += '</div>';

    html += '<div class="card"><div class="sectitle">النسخ المحفوظة (' + vers.length + ')</div>' +
      '<div style="font-size:11px;color:var(--text2);line-height:1.8;margin-bottom:10px">' +
      'لقطة كاملة للمستندات قبل كل تغيير. الاسترجاع يأخذ لقطة أخرى أولًا، فلا تفقد الحالة الراهنة.</div>';
    html += vers.length ? vers.slice(0, 25).map(function (v) {
      return '<div class="fileinfo" style="margin-bottom:8px">' +
        '<span>' + new Date(v.at).toLocaleString('ar-SA') + ' · ' + esc(v.actor) +
        ' · ' + v.docs.length + ' مستند' + (v.label ? ' · ' + esc(v.label) : '') + '</span>' +
        '<button type="button" data-vs="' + v.id + '">استرجاع</button></div>';
    }).join('') : '<div class="empty">لا توجد نسخ بعد.</div>';
    html += '</div>';

    html += '<div class="card"><div class="sectitle">قرارات سابقة</div>';
    html += hist.length ? hist.map(function (r) {
      var okc = r.status === 'approved';
      return '<div style="padding:9px 0;border-bottom:1px solid var(--line);font-size:12px;line-height:1.8">' +
        '<span class="badge ' + (okc ? 'low' : 'high') + '">' + (okc ? 'معتمد' : 'ملغى') + '</span> ' +
        esc(r.title) + ' — ' + esc(r.actor) +
        '<div style="font-size:10.5px;color:var(--text3)">' + new Date(r.decidedAt || r.at).toLocaleString('ar-SA') +
        ' · بقرار ' + esc(r.decidedBy || '—') + '</div></div>';
    }).join('') : '<div class="empty">لا قرارات بعد.</div>';
    html += '</div>';

    el.innerHTML = html;
    wire(el);
    on(el, '[data-ok]', 'click', function (b) { RQ.decide(b.dataset.ok, true); refreshAll(); renderApprovals(); });
    on(el, '[data-no]', 'click', function (b) {
      var r = RQ.decide(b.dataset.no, false);
      refreshAll(); renderApprovals();
      if (r.ok && !r.reverted) alert('أُلغي الإجراء، لكن لم توجد لقطة سابقة لإرجاعها.');
    });
    on(el, '[data-vs]', 'click', function (b) {
      if (!window.confirm('استرجاع هذه النسخة؟ ستُحفظ الحالة الحالية كنسخة قبل الاسترجاع.')) return;
      if (VS.restore(b.dataset.vs)) {
        AU.log({ kind: 'security', actor: S.session.name, title: 'استرجاع نسخة', detail: b.dataset.vs });
        refreshAll(); renderApprovals();
      }
    });
  }
